...
#stop ap74;docker-compose build ap74;fig up -d ap74;log ap74;php -m
#RUN NPROC=$(grep -c ^processor /proc/cpuinfo 2>/dev/null || 1)
VOLUME /sys/fs/cgroup
RUN apk update
RUN apk add git curl tar openssh openssl bash htop autoconf apache2 redis rsync make gcc mariadb-client mysql apache2-utils apache2-ssl apache2-proxy gzip sudo libressl nano vim libsodium-dev curl-dev libmcrypt-dev libmemcached-dev libpng-dev ${PHPIZE_DEPS}
RUN apk add openssh-server openrc && mkdir /var/run/sshd
RUN echo 'coucou';
RUN pecl channel-update pecl.php.net && pecl install apcu igbinary memcached redis mcrypt
...
RUN docker-php-ext-install mysqli && docker-php-ext-install pdo_mysql && docker-php-ext-install opcache && docker-php-ext-install sockets && docker-php-ext-enable sodium
...
RUN rm -rf ~/.pearrc && rm -rf /var/cache/apk && rm -rf /tmp && mkdir /var/cache/apk && mkdir /tmp && chmod -R 777 /tmp 
...
RUN apk del make gcc autoconf libsodium-dev curl-dev libmcrypt-dev libmemcached-dev libpng-dev ${PHPIZE_DEPS}
# -f /tmp/sshkey
